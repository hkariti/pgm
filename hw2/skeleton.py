...
def Q1():
    ...
    # complete
    prob_y = belief_prop.query(variables=['a', 'h', 'e'])
    print(prob_y)
    ...
    # complete
    prob_b = belief_prop.query(variables=['b', 'j', 'f'])
    print(prob_b)
...
def Q2():
    ...
    belief_prop = BeliefPropagation(G)
    num_ind_sets = belief_prop.query(variables=list(G.nodes))
    print((num_ind_sets.values > 0).sum())
    ...
    size_mis = sum(belief_prop.map_query(list(G.nodes)).values())
    print(size_mis)
